...
- An explicitly-flagged Outlook/Apple all-day event whose underlying start/end retain a non-midnight time (e.g. a recurring all-day event anchored at a fixed local time), and separately an ordinary timed event long enough to be auto-classified all-day by duration alone, could each remain visible in "today"'s relative-time list a full day after they ended, or get an extra day tacked onto their span with a spurious "ends" marker; exclusive-end handling now keys off an explicit marker set once at each event's origin (genuinely date-only, or an explicit all-day flag) instead of trying to infer it from the `allDay` flag or the end time's clock reading, both of which are shared with unrelated cases
- Google Calendar all-day events showing in the relative-time list displayed "NaN seconds ago" instead of a real duration, since their date-only string values were compared for filtering but never normalized before being handed to the countdown display
- A long-running event spanning into a day with no pre-built container could throw and abort the whole event-list refresh instead of just that event failing to render
- A recurring all-day event with no explicit Outlook/Apple flag (a plain date-only DTSTART) rendered as a timed event instead of all-day; a non-recurring event explicitly flagged all-day but short enough to miss the duration threshold had the same problem. Also: a literal (non-exclusive-end) event whose end happens to land exactly on local midnight - e.g. an event ending "tomorrow at 00:00" - was treated as still running into that day instead of having already ended
- Bumped the Google Calendar event cache key so a deploy doesn't keep serving stale cached events (missing newly-added fields) until the cache naturally expires
