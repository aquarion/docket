...
- An explicitly-flagged Outlook/Apple all-day event whose underlying start/end retain a non-midnight time (e.g. a recurring all-day event anchored at a fixed local time) could remain visible in "today"'s relative-time list a full day after it ended, or get an extra day tacked onto its span with a spurious "ends" marker; the exclusive-end handling now keys off an explicit marker for the unrelated "long timed event treated as all-day" case instead of guessing from the end time's clock reading
