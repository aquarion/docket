...
function logException($e)
{
    ...
    if (defined('SEND_JSON_ERRORS') && SEND_JSON_ERRORS == true) {
        ...
        echo "<tr style='background-color:rgb(230,230,230);'>";
        echo "<th style='width: 80px;'>Type</th><td>".get_class($e)
            .'</td></tr>';
        echo "<tr style='background-color:rgb(240,240,240);'>";
        echo "<th>Message</th><td>{$e->getMessage()}</td></tr>";
        echo "<tr style='background-color:rgb(230,230,230);'>";
        echo "<th>File</th><td>{$file}</td></tr>";
        echo "<tr style='background-color:rgb(240,240,240);'>";
        echo "<th>Line</th><td>{$e->getLine()}</td></tr>";
        echo '</table></div>';
        exit();
    }
}
...
function sendTextError($errno, $errstr, $errfile, $errline)
{
    ...
    echo "{$errstr} in {$file} at {$errline}";
    debug_print_backtrace();
    exit();
    // throw new ErrorException($errstr, $errno, 0, $errfile, $errline);
}
