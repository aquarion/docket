...
function XLSF(oTarget, urlBase) {
  ...
  if (window.innerWidth || window.innerHeight) {
    ...
    var screenX =
      ...
      document.body.scrollWidth; // -(!isIE?8:0);
    var screenY =
  ...
  if (window.location.href.match(/size=/i)) {
    this.lightClass = window.location.href.substr(
  ...
  function ExplosionFragment(nType, sClass, x, y, vX, vY) {
    ...
    this.o.style.backgroundPosition =
      this.w * -this.burstPhase + "px " + this.h * -nType + "px";
    ...
    this.burstTween = function () {
      ...
      var phase =
        1 +
        Math.floor((this.currentFrame / this.totalFrames) * self.burstPhases);
      if (phase != self.burstPhase) {
        self.burstPhase = phase;
        self.o.style.backgroundPosition =
          self.w * -self.burstPhase + "px " + self.h * -nType + "px";
      }
    };
    ...
    this.burst = function () {
      self.oA = new Y.A(
        self.o,
        { marginLeft: { to: self.vX * 8 }, marginTop: { to: self.vY * 8 } },
        animDuration,
        Y.UE.easeOutStrong,
      );
  ...
  function Light(sSizeClass, sClass, nType, x, y) {
    ...
    this.bgBaseY = !self.useY ? -this.h * this.nType : 0;
    ...
    this.pan = parseInt(
      this.x <= xlsf.soundPan.mid
        ? -panValue + (this.x / xlsf.soundPan.mid) * panValue
        : ((this.x - xlsf.soundPan.mid) /
            (xlsf.soundPan.right - xlsf.soundPan.mid)) *
            panValue,
    );
    ...
    this.setBGPos = function (x, y) {
      self.o.style.backgroundPosition =
        self.bgBaseX + x + "px " + (self.bgBaseY + y) + "px";
    };
    ...
    this.getDimensions = function () {
      ...
      self.bgBaseX = self.useY ? -self.w * this.nType : 0;
      self.bgBaseY = !self.useY ? -this.h * this.nType : 0;
    };
  ...
  this.rotateLights = function () {
    self.lights[
      self.lightIndex == self.lights.length
        ? self.lights.length - 1
        : self.lightIndex
    ].off();
    self.lightIndex++;
  ...
  this.destroyLight = function () {
    ...
    if (self.lightSmashCounter < self.lights.length) {
      var limit = Math.min(
        self.lightSmashCounter + groupSize,
        self.lights.length,
      );
  ...
  this.startSequence = function (fSequence, nInterval) {
    ...
    self.timer = window.setInterval(
      fSequence,
      typeof nInterval != "undefined" ? nInterval : self.lightInterval,
    );
  };
