...
var Radiator = {
	...
	init: function () {
		this.config.secondsPerRefresh =
			RadiatorConfig.constants.SECONDS_PER_REFRESH || 1800;
	...
	startTimers: function () {
		...
		this.intervals.push(
			window.setInterval(function () {
				if (
					self.circleProgress.trackPercent <=
					Radiator.constants.PROGRESS_THRESHOLD
				) {
					self.circleProgress.animate(
						self.circleProgress.trackPercent,
						"countdown",
					);
...
Radiator.ui = {
	...
	getTimeOfDay: function () {
		try {
			var now = new Date();
			var sunstate = SunCalc.getTimes(
				now,
				RadiatorConfig.constants.LATITUDE,
				RadiatorConfig.constants.LONGITUDE,
			);
...
Radiator.calendar = {
	...
	setup: function () {
		...
		fetch(
			"/all-calendars.php?end=" +
				...
				"&version=" +
				RadiatorConfig.constants.VERSION,
		)
		...
		if (RadiatorConfig.constants.ICAL_CALENDARS) {
			for (const [name, cal] of Object.entries(
				RadiatorConfig.constants.ICAL_CALENDARS,
			)) {
				this.updateIcal(
					cal.proxy_url,
					new Date(),
		...
		fetch(calendarUrl)
			...
			.then(function (data) {
				...
				var endRange = new Date(
					new Date().getTime() + msPerDay * approxDaysInMonth,
				);
				...
				for (var index = 0; index < eventComps.length; index++) {
					...
					if (
						RadiatorConfig.constants.FILTER_OUT_LIST &&
						RadiatorConfig.constants.FILTER_OUT_LIST.indexOf(summary) !== -1
					) {
					...
					if (RadiatorConfig.constants.FILTER_OUT_REGEXES) {
						for (const regexString of RadiatorConfig.constants
							.FILTER_OUT_REGEXES) {
					...
					if (event.isRecurring()) {
						Radiator.calendar.processRecurringEvent(
							item,
							event,
							duration,
							rangeStart,
						...
						Radiator.calendar.processSingleEvent(
							item,
							allDayMinutes,
							name,
							events,
	...
	processRecurringEvent: function (
		...
		while (next) {
			...
			var allDay = this.determineAllDay(
				item,
				minutesLength,
				allDayMinutes,
				title,
...
Radiator.events = {
	...
	mergeAllDayEvents: function (data) {
		...
		for (var i = 0; i < data.allday.length; i++) {
			...
			var existingEvent = mergedAllday.find(function (event) {
				return event.title === currentEvent.title;
			});
