...
updateNextUp = function(){
      ...
      if (this_event.allDay){
          ...
          if(days[startF]){
              ...
              if (durationHours > 0){
                  ...
                  x_event.title = x_event.title + " (until " + x_end.calendar(UNTIL_CALENDAR_FORMAT) + ")";
              }
  ...
  for (const [date, data] of Object.entries(days)) {
      ...
      for (; allday_index < data.allday.length; allday_index++) {
        let mergedAllday = [];
        for (let allday_index = 0; allday_index < data.allday.length; allday_index++) {
            const currentEvent = data.allday[allday_index];
            const existingEvent = mergedAllday.find(event => event.title === currentEvent.title);
            if (existingEvent) {
...
function update_datetime(){
  ...
  $('#date').html(strToday);
  $('#time').html(time);
  $('#datetime').html("<div class=\"dt_time\">" + time + "</div><div class=\"dt_date\">" + strToday + "</div>");
}
    ...
    var eventComps = false;
      ...
      if (comp.getFirstSubcomponent('vtimezone')) {
          for (const tzComponent of comp.getAllSubcomponents('vtimezone')) {
              debug("Registering Timezone: " + tzComponent.getFirstPropertyValue('tzid'));
