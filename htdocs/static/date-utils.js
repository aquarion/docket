...
function fromNow(date) {
  ...
  if (diff > 0) {
    return 'in ' + humanizeDuration(absDiff);
  } else {
    return humanizeDuration(absDiff) + ' ago';
  }
}
...
function calendarFormat(date) {
  ...
  if (diffDays === 0) {
    ...
    return formatDate(date, 'ddd D') + ' at ' + formatDate(date, 'HH:mm');
  } else {
...
function formatDateWithOrdinal(date) {
  ...
  return parts.weekday + " " + parts.month + " " + parts.day + 
         "<sup>" + dateOrdinal(parseInt(parts.day)) + "</sup>";
}
 ...
 */
function findFurthestDate(events) {
  let max = new Date();
